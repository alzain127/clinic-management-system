...
@section('content')
    <div class="page-header">
        <h1>{{ isset($invoice) ? 'تعديل الفاتورة #' . $invoice->id : 'إنشاء فاتورة جديدة' }}</h1>
    </div>
        ...
        <div class="card-body">
            <form method="POST"
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror"
                            id="amount" name="amount" value="{{ old('amount', $invoice->amount ?? '') }}" required min="0">
                        @error('amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('payment_status') is-invalid @enderror" id="payment_status"
                            ...
                            <option value="غير مدفوع" {{ old('payment_status', $invoice->payment_status ?? '') == 'غير مدفوع' ? 'selected' : '' }}>غير مدفوع</option>
                            <option value="مدفوع" {{ old('payment_status', $invoice->payment_status ?? '') == 'مدفوع' ? 'selected' : '' }}>مدفوع</option>
                            <option value="مدفوع جزئياً" {{ old('payment_status', $invoice->payment_status ?? '') == 'مدفوع جزئياً' ? 'selected' : '' }}>مدفوع جزئياً</option>
                        </select>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button type="submit"
                        class="btn btn-primary">{{ isset($invoice) ? 'تحديث الفاتورة' : 'حفظ الفاتورة' }}</button>
                </div>
            </form>
        </div>
    </div>
