...
@section('content')
    <div class="page-header">
        <h1>{{ isset($patient) ? 'تعديل بيانات المريض' : 'إضافة مريض جديد' }}</h1>
    </div>
        ...
        <div class="card-body">
            <form method="POST"
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('birth_date') is-invalid @enderror" id="birth_date" name="birth_date" value="{{ old('birth_date', isset($patient) && $patient->birth_date ? $patient->birth_date->format('Y-m-d') : '') }}" required>
                        @error('birth_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <textarea class="form-control @error('medical_history') is-invalid @enderror" id="medical_history" name="medical_history" rows="4">{{ old('medical_history', $patient->medical_history ?? '') }}</textarea>
                ...
                <button type="submit" class="btn btn-primary">{{ isset($patient) ? 'تحديث البيانات' : 'حفظ المريض' }}</button>
            </div>
            </form>
        </div>
    </div>
